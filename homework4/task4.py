"""
Write a function that takes a number N as an input and returns N FizzBuzz numbers*
Write a doctest for that function.
Write a detailed instruction how to run doctests**.

That how first steps for the instruction may look like:
 - Install Python 3.8 (https://www.python.org/downloads/)
 - Install pytest `pip install pytest`
 - Clone the repository <path your repository>
 - Checkout branch <your branch>
 - Open terminal
 - ...


Definition of done:
 - function is created
 - function is properly formatted
 - function has doctests
 - instructions how to run doctest with the pytest are provided

You will learn:
 - the most common test task for developers
 - how to write doctests
 - how to run doctests
 - how to write instructions


>>> fizzbuzz(5)
['1', '2', 'fizz', '4', 'buzz']

* https://en.wikipedia.org/wiki/Fizz_buzz
** Энциклопедия профессора Фортрана page 14, 15, "Робот Фортран, чисть картошку!"
"""
from typing import List


def fizzbuzz(n: int) -> List[str]:
    """
    Taking a number and giving list of numbers as a strings from 1 till the number
    with replacing numbers divisible by 3 on fizz and divisible by 5 on buzz
    and divisible by 3 and by 5 on fizzbuzz.

    >>> fizzbuzz(5)
    ['1', '2', 'fizz', '4', 'buzz']

    >>> fizzbuzz(0)
    []

    >>> fizzbuzz(15)
    ... # doctest: +NORMALIZE_WHITESPACE
    ['1', '2', 'fizz', '4', 'buzz', 'fizz', '7', '8', 'fizz',
    'buzz', '11', 'fizz', '13', '14', 'fizzbuzz']

    """
    lst = []
    for number in range(1, n + 1):
        if number % 15 == 0:
            lst.append("fizzbuzz")
        elif number % 3 == 0:
            lst.append("fizz")
        elif number % 5 == 0:
            lst.append("buzz")
        else:
            lst.append(str(number))
    return lst
