...
def reading_text_in_lines(file_path: str) -> List[str]:
...
def reading_text(file_path: str) -> str:
...
def get_longest_diverse_words(file_path: str) -> List[str]:
    ...
    text = reading_text_in_lines(file_path)
    punctuation = (".", ":", ",", "?", "!")
    longest_words = []
    for line in text:
        line = line.strip().encode().decode("unicode_escape").split()
        for word in line:
...
def count_punctuation_chars(file_path: str) -> int:
    ...
    punctuations = [",", ".", ":", ";", "?", "!", "(", ")", "—"]
    acc = 0
    text = reading_text(file_path).encode().decode("unicode_escape")
    for char in text:
...
def get_most_common_non_ascii_char(file_path: str) -> str:
    ...
    text = reading_text_in_lines(file_path)
    for line in text:
        line = line.strip().encode().decode("unicode_escape")
        for char in line:
            if not char.isascii():
                if char in frequency:
                    frequency[char] += 1
